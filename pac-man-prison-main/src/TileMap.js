...
export default class TileMap {
    ...
    draw(ctx) {
        for (let row = 0; row < this.map2.length; row++) {
            for (let column = 0; column < this.map2[row].length; column++) {
                let tile = this.map2[row][column];
                if (tile === 1) {
    ...
    getEnemies(velocity) {
        ...
        for (let row = 0; row < this.map2.length; row++) {
            for (let column = 0; column < this.map2[row].length; column++) {
                const tile = this.map2[row][column];
                if (tile == 5) {
                    this.map2[row][column] = 8;
                    enemies.push(
                        new Enemy(
                            column * this.tileSize,
                            row * this.tileSize,
    ...
    setCanvasSize(canvas) {
        canvas.width = this.map2[0].length * this.tileSize;
        canvas.height = this.map2.length * this.tileSize;
    };
    ...
    #dotsLeft() {
        return this.map2.flat().filter((tile) => tile === 0).length;
    }

    #fagsLeft() {
        return this.map2.flat().filter((tile) => tile === 7).length;
    }

    #phonesLeft() {
        return this.map2.flat().filter((tile) => tile === 8).length;
    }

    #cashLeft() {
        return this.map2.flat().filter((tile) => tile === 9).length;
    }
